...
export default function SideNavigation() {
    ...
    return (
        <motion.nav
            className="fixed left-4 md:left-8 top-1/2 -translate-y-1/2 z-50 bg-foreground/95 backdrop-blur-md p-3 md:p-6 md:rounded-3xl rounded-full shadow-xl border border-secondary/30 flex flex-col gap-6"
            initial={{ opacity: 0, x: -50 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.8, delay: 0.5 }}
        >
            <div className="hidden md:block text-white font-bold text-lg text-center mb-4 border-b border-secondary/30 pb-4">
                Le Temple<br />du Geek
            </div>
            ...
            {navItems.map((item) => (
                <div key={item.id} className="relative group">
                    <button
                        ...
                        className="flex items-center gap-4 p-3 md:px-4 md:py-3 w-full rounded-full md:rounded-xl text-secondary font-bold hover:bg-secondary/20 hover:text-secondary transition-all duration-300"
